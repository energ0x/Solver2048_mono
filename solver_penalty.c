...
static double evaluate(uint64_t b) {
    if (get_max_log(b) >= 11) return -1000000.0;   /* абсолютна заборона */
...
void solver_play_game(uint64_t seed, int base_depth,
    ...
    while (1) {
        int d = dynamic_depth(b, base_depth);
        int mv = search_root_depth(b, d);
        if (mv == -1) break;                 /* немає безпечного ходу */
